...
const nav = [['Home', '/#home'], ['About', '/about'], ['Our Work', '/work'], ['Events', '/events'], ['Contact', '/contact']];
const services = [
...
function Footer() { return <footer className="footer"><div className="container"><div className="footer-grid"><div><Logo /><p>Creating remarkable events and experiences from concept to execution.</p></div><div><h4>Explore</h4>{nav.slice(0, 5).map(([n, id]) => <a href={id.startsWith('/#') ? id.substring(1) : id} key={id}>{n}</a>)}</div><div><h4>Contact</h4><a href="tel:+919819866075">+91 98198 66075</a><a href="mailto:mrlevents2023@gmail.com">mrlevents2023@gmail.com</a><span>1/1/18, Lotus CHS Ltd., Bhawani Nagar, Marol, Andheri (E), Mumbai – 400059</span></div><div><h4>Social</h4>{[{name: 'Instagram', url: 'https://www.instagram.com/eventsmrl/'}, {name: 'Facebook', url: 'https://www.facebook.com/p/MRL-Events-100093348081655/'}].map(s => <a href={s.url} key={s.name} target="_blank" rel="noopener noreferrer">{s.name}<ArrowUpRight /></a>)}</div></div><div className="footer-bottom"><span>© 2026 MRL Events. All rights reserved.</span><div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}><Link to="/privacy-policy">Privacy Policy</Link><Link to="/terms">Terms & Conditions</Link><Link to="/refund-policy">Refund Policy</Link></div></div></div><div className="footer-line" /></footer> }
